...
9. [Compras do Dia](#9-compras-do-dia)
10. [Relatórios](#10-relatórios)
11. [Geração de Recibos PDF](#11-geração-de-recibos-pdf)
12. [Pré-visualização de Recibos](#12-pré-visualização-de-recibos)
13. [Interface Gráfica](#13-interface-gráfica)
14. [Implementação do CRUD](#14-implementação-do-crud)
15. [Estruturas de Dados Utilizadas](#15-estruturas-de-dados-utilizadas)
16. [Controlo de Permissões](#16-controlo-de-permissões)
17. [Tecnologias Utilizadas](#17-tecnologias-utilizadas)
18. [Resultados Obtidos](#18-resultados-obtidos)
...
Com o crescimento das atividades comerciais e a necessidade de um controlo cada vez mais eficiente das operações de venda, torna-se essencial a utilização de sistemas informáticos capazes de automatizar processos, reduzir erros e melhorar a gestão da informação.

Neste contexto, foi desenvolvido o sistema **AQStore – Loja Aquapet Lda**, uma aplicação desktop destinada à gestão comercial de uma loja de produtos para animais de estimação. O sistema foi concebido com o objetivo de facilitar o registo de vendas, a gestão de produtos, o controlo de utilizadores e a geração de relatórios, proporcionando uma solução integrada para apoio às operações diárias da empresa.

O projeto foi desenvolvido utilizando a linguagem de programação **Java**, recorrendo à framework **JavaFX** para construção da interface gráfica, à base de dados **MySQL** para armazenamento permanente das informações e à tecnologia **JDBC** para comunicação entre a aplicação e a base de dados. Foram ainda utilizadas bibliotecas complementares, como **OpenPDF** para geração de recibos em formato PDF, **Apache PDFBox** para pré-visualização dos documentos gerados e **Apache POI** para exportação de relatórios em formato Excel.
...
Durante o desenvolvimento foram aplicados conceitos de **Programação Orientada a Objetos**, **Estruturas de Dados**, **Bases de Dados Relacionais**, **Interfaces Gráficas** e **Engenharia de Software**, permitindo consolidar conhecimentos adquiridos ao longo do curso de Engenharia Informática.

Além do aspeto técnico, o projeto procurou responder a uma necessidade real de gestão comercial, disponibilizando uma ferramenta capaz de melhorar a organização dos processos internos, aumentar a produtividade dos utilizadores e garantir maior controlo sobre as informações da empresa.

Desta forma, o sistema AQStore constitui uma solução informática completa para apoio à gestão da Loja Aquapet Lda, integrando diferentes tecnologias e demonstrando a aplicação prática dos conhecimentos académicos adquiridos ao longo da formação.
...
Define a palavra-passe utilizada para autenticação.
...
A operação CREATE é responsável pelo cadastro de novos produtos.
...
Esta consulta adiciona um novo registo à tabela `produtos`.

Os valores são enviados através do `PreparedStatement`, aumentando a segurança da aplicação.
...
Após executar a operação, o produto ficará registado na base de dados.
...
# 12. Pré-visualização de Recibos
...
E responsável por transformar páginas PDF em imagens.
...
O desenvolvimento deste projeto permitiu não apenas a criação de uma aplicação funcional, mas também o reforço de competências técnicas relacionadas com:
...
A experiência adquirida durante a implementação do AQStore constitui uma importante contribuição para a formação profissional e académica dos membros envolvidos no projeto, preparando-os para desafios futuros na área do desenvolvimento de sistemas de informação.